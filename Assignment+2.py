...
def answer_three():
    copy_df = df.copy()
    copy_df = df.where(df['Gold']>0) 
    copy_df = df.where(df['Gold.1']>0)
    df['relative'] = ((copy_df['Gold']-copy_df['Gold.1'])/copy_df['Gold.2'])
    (df['relative'].max())
...
def answer_six():
    copy_census = census_df.copy()
    copy_census = copy_census[copy_census['SUMLEV'] == 50].groupby(['STNAME'])
    Pop = copy_census['CENSUS2010POP'].apply(lambda x: x.nlargest(3).sum()).nlargest(3).index.values.tolist()
    return Pop
...
def answer_seven():
    ...
    ABS = (pop.max(axis=1)-pop.min(axis=1)).argmax()
    return census_df.loc[ABS]['CTYNAME']
